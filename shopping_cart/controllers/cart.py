...
async def validate_product(cart_data=CartRequestSchema, raise_exception=False):

    product = await search_product_by_id(cart_data.product_id)

    if product['stock'] < cart_data.quantity and raise_exception:
        raise NotAvailableException('Produto indisponível para essa quantidade')
    return product


async def validate_item(email: EmailStr, product_id: int, raise_exception=False):
...
async def create_user_cart(user: dict):
    await cart_crud.create_cart(UserSchema(**user))
    cart = await fetch_cart_by_email(user['email'])
    return cart

async def add_cart_item(email: EmailStr, cart_data: CartRequestSchema):
...
async def remove_cart_item(email: EmailStr, cart_data: CartRequestSchema):
    
    # Valida se o usuário tem carrinho aberto
    if not await has_cart(email):
        raise("User's shopping cart is empty")

    cart = await validate_item(email, cart_data.product_id, raise_exception=True) # Valida se o item está no carrinho e retorna o carrinho
    await validate_quantity_to_remove(cart, cart_data.product_id, cart_data.quantity) # Valida se a quantidade a ser removida é maior do que a quantidade d eitem do carrinho
    product = await validate_product(cart_data) # Valida se o produto existe e retorna o produto
...
async def clean_user_cart(email: EmailStr):
    if not await has_cart(email):
...
async def validate_quantity_to_remove(cart: dict, product_id: int, quantity: int):
    ...
    if document_quantity < quantity:
        raise NotValidException(
            "Quantidade inválida. A quantidade inserida é superior a existente no carrinho"
        )
...
async def update_item_quantity(
    ...
    index = 0
    for item in cart_document['order_items']:
