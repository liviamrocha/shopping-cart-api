...
def test_router_get_by_email_fail(mock_user_by_email, mock_find_cart_by_email, client_authenticated):
    mock_user_by_email.return_value = {"email": "email"}
    mock_find_cart_by_email.return_value = {"user.email": "email"}

    respost = client_authenticated.get(f"/carts/?email=")
